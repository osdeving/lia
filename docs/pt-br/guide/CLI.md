...
- `lia lower <file.lial> --target java --profile plain|spring-boot|quarkus [--strict] --out-dir out-dir`
  ...
  - `--strict`: falha em vez de gerar placeholder ou adapter stub
...
- `lia gen app --prompt "..." --provider ollama|openai-compatible --model <model> --out-dir <dir> [--java-profile plain|spring-boot|quarkus] [--java-profiles plain,spring-boot,quarkus] [--java-strict]`
  ...
  - `--java-strict` impede placeholder/stub no lower Java
...
- `lia demo compare --spec project-spec.json --provider ollama|openai-compatible --model <model> --out-dir <dir> [--compare-profile spring-boot] [--lia-java-profiles plain,spring-boot,quarkus]`
  ...
  - o ramo LIA da demo roda com lower estrito
